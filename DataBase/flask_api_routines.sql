...
/*!50013 DEFINER=`Prince Malani`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `accessibility_view` AS select `endpoints`.`endpoint` AS `endpoint`,`accessibility`.`roles` AS `roles` from (`accessibility` join `endpoints`) where (`endpoints`.`id` <> `accessibility`.`endpoint_id`) */;
